...
class FoodCouponsVC: UIViewController,UITableViewDataSource,UITableViewDelegate {
    ...
    func check(){
        ...
        for key in typeArr {
            Database.database().reference().child("scannables").child("attendances").child(key).child("couponUidList").child(Auth.auth().currentUser!.uid).observeSingleEvent(of: .value) { (snapshot) in
                if(snapshot.exists()){
                    self.isPresent = true
                    reedeemedArr.append(key)
                }
        }
        }
    }
